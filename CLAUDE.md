...
- Z 규약: Z=0 이 맨 위(들어 올린 자리), 값이 커질수록 아래(펄스 부호도 +가 아래).
  ...
  Z_MAX 6880(43mm)은 실측값, Z_PPMM 160 은 아직 가정이다(고치면 펌웨어·core/stage.py 둘 다).
...
- 끝단 이동 거리는 1~248 mm. 펌웨어 jx/jy 한도(8000 펄스 = 50 mm)는 그대로 두고
...
- 수동 이동 팝업은 위에서 아래로 [위치] [조그] [좌표 이동] [원점 잡기] [비상정지].
  ...
  [Z↑][Z↓](PageUp/PageDown), 좌표 이동에 [Z 맨 위로](z_top).
...
3. Z축·프로브 — 펌웨어 V9·드라이버·조그·원점 등록까지 완료. 남은 것: X·Y 와의
   인터록(Z 가 내려가 있으면 X·Y 금지, 이동 전 Z 상승 강제)과 순회에 Z 하강·상승
   넣기. 둘 다 프로브를 단 뒤에 한다. Z_MAX 는 실측(43mm), Z_PPMM 은 아직 가정값이다.
