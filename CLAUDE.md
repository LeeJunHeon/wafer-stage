...
중앙값 배경 나눗셈(nearest-fill) → 히스테리시스(seed 15%/grow 12%) + 채도 양방향 OR(포화 픽셀 제외) **OR 통계 씨앗**(stat_path: σ=stat_blur 블러 → Lab 채널별 중앙값 배경 잔차 → 채널 잡음 σ(ROI 잔차 MAD×1.4826, 하한 1.0 = 8비트 Lab 양자화 한 단계) → d²=Σ(r/σ)² > stat_seed 25 / stat_grow 12, χ²(3)) → 부호 분리(L 잔차 부호로 붙은 덩어리 가르기. 통과 덩어리는 둘 이상 나올 때만, 띠에 닿은 저볼록도 덩어리는 부호 분리만) → rescue/split(홈 짝 선택·ㄱ자 단일 홈 컷·재귀) → 병합 → 모양 판정 + 얼룩·림 관문 → 색거리(dE) 보조 경로 → 엣지 보완(3.7, 새 샘플 없음) → 엣지 제안(3.8, 새 샘플. edge_propose).
통계 씨앗은 기존 seed·grow 를 대체하지 않고 OR 로 더한다(2026-09-18 정정). 어두운 사진(142900: 평탄화 뒤 웨이퍼 밝기 45)에서만 이득이고 밝은 사진(143334)에서는 대체하면 12/16 으로 떨어졌다. seed_pct·grow_pct·sat_delta·de_* 는 그대로 쓴다.
...
엣지 제안(3.8): 같은 재질의 짙은 회색 칩은 면 대비가 0(Lab 잔차 < 잡음)이라 가장자리 선만 보인다. σ=1 블러한 L·a·b Canny OR(edge_canny 사다리 전부. 둘째 단계부터 직선성 edge_straight_loose 0.90) → 3×3·5×5 닫기 → 컨투어 → 볼록껍질 → 면적 edge_min_area_mm2(25)~max → 꼭짓점 3~edge_vertices_max → 직선성 ≥ edge_straight_min 0.85 → 엣지 지지율(껍질 둘레 2px 안 엣지 비율 ≥ edge_support_min 0.6) → 볼록도(edge_solidity_min. 지지율 ≥0.85 면 면제 - ㄷ자 열린 엣지) → 안쪽 비어 있음(엣지 비율 ≤ edge_inner_max, d² 중앙값 < stat_grow) → 안쪽이 웨이퍼보다 밝지 않음(밝은 얼룩 제외 - 143334 가운데 얼룩) → 포화 화소 3px 안 없음 → 기존 샘플과 겹침 ≤ edge_overlap_max(양방향) → 테두리 띠에 닿지 않음. 들어온 샘플은 edge_only 'G' + "found by edge only; verify".
...
surface 모드 wafer/plate/auto. 검출 로직을 고치면 반드시 `python tools/regress.py` 로 개수 회귀를 본다(브라케팅 촬영은 fused.png 로 돈다). 회귀 기준은 개수가 아니라 '맞는 자리': `tools/flat_check.py X/flat.png --truth self`(samples.json 을 정답으로, 같은 사진이면 px 로 2.5mm 환산) - 143334 는 16/16·오검출 0 을 지켜야 하고, 143454 는 적중 ≥14·오검출 ≤2, 142900 은 ≥13. 경로별 기여는 `--mode all`.
