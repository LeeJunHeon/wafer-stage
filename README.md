...
| 명령 | 내용 |
|---|---|
| `jx <±펄스>` `jy <±펄스>` | **원점 없이도 되는 상대 이동**. 한 번에 8000 펄스(50mm)까지 — 더 먼 거리는 드라이버가 조각으로 나눠 보낸다. 가동범위 밖으로 나가면 그 축 원점 해제 |
...
| `stage_connect` | `port?` |
| `stage_disconnect` | |
| `touch_end` | `axis:"x"\|"y"`, `mm`(기본 5, 1~248) — 그 축을 끝단 쪽으로 민다. 50 mm 조각으로 나눠 보낸다. 등록하지 않는다 |
| `set_origin` | `axis:"x"\|"y"\|"xy"` — 끝단에서 2 mm 물러난 자리를 0 으로 등록(거리는 고정) |
| `park` | |
| `goto` | `no` 또는 `x, y` |
