...
- **기존 경로를 대체하지 않고 OR 로 더한다**(`stat_path`, 기본 켬). 대체했더니 조명을 켠
  밝은 사진(143334)에서 12/16 으로 떨어졌다 — 통계 씨앗은 어두운 사진(142900)에서만 이득이다.
  `seed_pct`·`grow_pct`·`sat_delta`·`de_floor`·`de_k`·색거리(dE) 보조 경로는 그대로다.
...
**회귀 기준은 '맞는 자리'다**: `python tools/flat_check.py DIR/flat.png --truth self` 가
samples.json 을 정답으로 적중·오검출·놓침을 찍는다(같은 사진이면 px 로 비교, 2.5 mm 환산 —
촬영마다 마커 재보정이 달라 기계좌표는 같은 칩도 몇 mm 씩 어긋난다). 2026-09-18 결과:
143334 **16/16 · 오검출 0**(이걸 못 지키는 변경은 넣지 않는다) · 143454 적중 15(옛 14 +
반사광 자리의 밝은 칩 (659,238)) · 오검출 2(옛것 그대로) · 142900 13(옛 12 + A(611,270)) ·
오검출 0. 142900 의 얼룩(633,290)은 안 잡힌다. B(702,277)는 엣지 지지율 0.65 · 직선성 0.76 ·
볼록도 0.35 로 관문에 못 든다. `tools/regress.py`(26장) 개수 감소 없음.
