...
`Z_MAX`(6880 = 43 mm)는 실측값이다 — 위 끝단에서 2 mm 이격한 0 에서 아래 끝단까지
약 45 mm 인데 아래도 2 mm 를 남겼다. `Z_PPMM`(160)은 아직 **가정값**이다.

### Z 실측 (프로브를 달기 전에)

1. 수동 이동 팝업에서 스텝 10 으로 [Z↓] 를 다섯 번 — 50 mm 를 지시한 셈이다.
2. 실제로 내려온 거리를 자로 잰다. `Z_PPMM = 160 x (지시 50) / (실제 mm)`.
   펌웨어 `Z_PPMM` 과 `core/stage.py` 의 `Z_PPMM` 을 같은 값으로 고친다.
3. ~~스트로크 측정~~ — 완료(43 mm = 6880 펄스). `Z_PPMM` 을 고치면 같은 스트로크의
   펄스 수도 달라지므로 `Z_MAX` 를 다시 계산해야 한다.
4. [Z↓] 인데 위로 올라가면 펌웨어의 `Z_DIR_INVERT` 를 `true` 로 바꾸고 다시 올린다.

Z 는 아직 X·Y 와 인터록이 없는 **독립 축**이다(X·Y 원점이 없어도 움직이고, Z 가
내려가 있어도 X·Y 를 막지 않는다). 인터록과 순회 중 Z 하강·상승은 프로브를 단 뒤에
넣는다(맨 아래 [미결]).
...
| `z_top` | Z 를 0(맨 위)으로. Z 원점이 있어야 한다. X·Y 원점과 무관 |
...
1. **Z 인터록** — 지금 Z 는 X·Y 와 아무 관계가 없다. 프로브를 달면 Z 가 내려가 있는
   동안 X·Y 이동을 막고, X·Y 이동 전에 Z 를 올리도록 서버가 강제해야 한다.
2. **순회에 Z 넣기** — [Z 상승 → X·Y 이동 → Z 하강 → 측정 → Z 상승] 자리를 engine 에
   비워 두었다. 하강 깊이는 설정값이 된다.
3. **Z 펄스/mm 실측** — 스트로크는 쟀다(43 mm). `Z_PPMM`(160)은 아직 가정값이라
   자로 재서 확인해야 한다(위 [Z 실측] 절차 1~2).
