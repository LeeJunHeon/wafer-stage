...
| `fz x [펄스]` `fz y [펄스]` | 끝단 맞춤. 인자가 없으면 800 펄스(5mm)만 민다. 1600(10mm) 초과는 거부 |
...
4. 더 정밀하게는 끝단 5 mm 이내에서 [끝단 맞춤] — 입력한 거리만큼만 밀고 물러나 0 으로 등록한다.
...
| `stage_connect` | `port?` |
| `stage_disconnect` | |
| `home_touch` | `axis`, `search_mm`(기본 5, 최대 10) — 끝단 맞춤 |
