...
1. **노출 브라케팅 + 융합** (`core/camera.py capture_bracket`, 설정 `bracket` 기본 켬)
   — 자동 노출을 끄고 노출을 바꿔 가며 받아 `cv2.createMergeMertens()` 로 융합한다
...
브라케팅했을 때) · `flat.png`(평탄화본, 적용됐을 때) · `annotated.jpg`. 평탄화는 포화
화소를 255 로 남긴다 — 나눗셈으로 값이 내려가면 검출의 포화 제외(V<250)가 무력화된다.
...
- σ=`stat_blur`(1.0) 블러 → Lab 세 채널 각각 국소 배경(`_local_bg`, 중앙값·nearest-fill)을
  ...
  양자화돼 MAD 가 0 이 된다) → d² = Σ(r/σ)² → 히스테리시스 `stat_seed`(25, χ²(3) p≈1e-5) /
  `stat_grow`(12, p≈7e-3). 설정 창에는 이 두 키만 보인다.
- `seed_pct`·`grow_pct`·`sat_delta`·`de_floor`·`de_k` 는 더 이상 쓰지 않는다(파일에
  남아 있어도 무시). 색거리(dE) 보조 경로와 "found by color only" 경고는 없앴다.
- 붙은 덩어리: L 잔차의 부호(배경보다 밝다/어둡다)로 가른다 — 중앙값 배경이 가까운 두
  ...
  셋이 붙은 것은 재귀로 가른다.
- 샘플마다 `strength`(덩어리 안 d² 중앙값)를 남기고 경로별 하위 `weak_frac`(20%)은
  `weak` 로 표시한다 — annotated 자홍 '?', 목록 '?' 배지. confirm 모드에서 사람이 본다.
...
**엣지 제안 경로** (`edge_propose`, 기본 켬): 웨이퍼와 같은 재질의 짙은 회색 칩은 Lab
...
142900/flat.png 결과: 통계 경로만 14개(기존 12 + C·D + 얼룩에 붙어 빠졌던 황갈색 칩;
부호 분리로 갈라졌다), 얼룩(633,290)은 안 잡힌다. A(612,272)는 d² 중앙값 6(grow 12) 이라
통계로는 안 잡히고 엣지도 2~12mm² 조각뿐이다(fused 를 다시 평탄화한 사진에서는 약하게
잡힌다). B(702,277)는 엣지 지지율 0.65 · 직선성 0.76 · 볼록도 0.35 로 관문에 못 든다.
`tools/regress.py`(26장): 개수가 준 사진은 153512 뿐이고 빠진 셋은 한 칩의 중복 검출 둘과
반사광 자락이다. 늘어난 것은 대부분 weak/edge_only 로 표시된다.
